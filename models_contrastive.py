...
class VisionTextModel(nn.Module):
    ...
    def __init__(self, vision_model, text_model, vision_dim, text_dim):
        ...
        self.vision_proj = nn.Linear(vision_dim, 512)  # Project vision features to shared space
        self.text_proj = nn.Linear(text_dim, 512)     # Project text features to shared space
        self.temperature = nn.Parameter(torch.tensor(0.07))  # Learnable temperature for InfoNCE        
        ...
        def contrastive_loss(self, vision_features, text_features, temperature=0.07):
            ...
            logits = torch.matmul(vision_embed, text_embed.T) / temperature  # [batch, batch]
    ...
    def forward(self, pixel_values, input_ids, attention_mask):
        ...
        try:
            ...
            # Compute text features for contrastive loss (e.g., mean of text embeddings)
            text_features = torch.mean(text_embed, dim=1)  # [batch, text_dim]

            # Compute contrastive loss
            cont_loss = self.contrastive_loss(vision_features, text_features, self.temperature)
                        
            logger.info("Text model forward pass completed")
            # return outputs #Uncomment this for not using cons loss and just use the normal loss
            return {
                'outputs': outputs, 
                'lm_loss': lm_loss, 
                'cont_loss': cont_loss,
                'total_loss': lm_loss + cont_loss
            }
